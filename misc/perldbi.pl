...
my $recs_= $dbh-> selectall_arrayref("SELECT val1 as keyval, val2 as dataval FROM vals", { Slice=> {} } } ;
...
while ( my ($x,$y,$z)= $sth-> fetchrow_array ) {
    ...
    #### as part of statment
{
  my $sth= $dbh-> prepare("SELECT val1 FROM vals") ;
  $sth-> execute() ;
  $sth-> rows() ;
  my @vals= @{ $sth-> fetchall_arrayref( [0] ) } ;
}

{
  my $sth= $dbh-> prepare("SELECT val1 FROM vals") ;
  $sth-> execute() ;
  my $recs_= $sth-> fetchall_arrayref( {} ) ;
}
